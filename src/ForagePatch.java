...
public class ForagePatch extends Cell{
    ...
    private void initializeAnts() {
        if (myCurrentState == ForageState.NEST){
            for (int i = 0; i < (int) (double) myParameters.getOrDefault(ForageSimulation.NUMBER_OF_ANTS, 10.0); i++){
    ...
    private void initializePheromones() {
        ...
        switch ((ForageState) myCurrentState) {
            ...
            case FOOD:
                myHomePheromones = 0.0D;
                myFoodPheromones = myParameters.getOrDefault(ForageSimulation.MAX_FOOD_PHEROMONES, 10.0);
            ...
            case EMPTY:
                ...
                break;
            case NEST:
                myHomePheromones = myParameters.getOrDefault(ForageSimulation.MAX_HOME_PHEROMONES, 10.0);
                myFoodPheromones = 0.0D;
                break;
        }
    }
    ...
    public void topOffPheromones(ForageState homeOrFood){
        if (homeOrFood == ForageState.NEST)
            myHomePheromones = myParameters.getOrDefault(ForageSimulation.MAX_HOME_PHEROMONES, 100.0D);
        else if (homeOrFood == ForageState.FOOD){
            myFoodPheromones = myParameters.getOrDefault(ForageSimulation.MAX_FOOD_PHEROMONES, 100.0D);
        }
    }
